...
Within a single stint, TyreLife and LapNumber are *exactly* collinear, not just nearly — TyreLife = LapNumber − pit_lap + 1 is a constant offset for the whole stint, so the design matrix `[1, TyreLife, LapNumber]` has rank 2, not 3. `lstsq` still returns a solution (the minimum-norm one), but the split between β_tyre and β_fuel within that solution is arbitrary; only their sum is identified from a single stint's data.

This is fixed via a two-stage regression (`fit_race_fuel_effect` + the `beta_fuel` argument to `fit_stint_deg` in [analysis/tyre_deg.py](analysis/tyre_deg.py)): β_fuel is estimated once per driver across their whole race using cross-stint variation — TyreLife resets to 1 at each pit stop while LapNumber keeps counting up, so with a single race-level intercept the TyreLife/LapNumber offset differs stint to stint and the collinearity breaks. That fixed β_fuel is then subtracted out of each stint's lap times before regressing the remainder on TyreLife alone, giving a clean single-regressor deg_rate per stint. `driver_deg_profile` wires this up automatically; races with fewer than two stints for a driver fall back to the old joint single-stint fit (still exact-collinear, sum-only identified) since there's no cross-stint variation to draw on.
...
The core idea is that if the spread between two drivers isn't stationary it has no natural mean to revert to, making the pairs strategy meaningless. The ADF tests whether a unit root exists — a unit root means shocks are permanent rather than mean reverting. If it doesn't, then the system becomes a random walk, basically invalidating the comparison strategy we use in the data. The ADF  tests the null hypothesis that a unit root exists — in other words the default assumption is that the series is non-stationary
